(function ($) {
    ...
    JqPort.prototype = {
    	...
    	initHtml:function(data,url){
    		...
    		if(data.length==0){
	    		...
	    		$.each(data, function (i, data) {
	    			...
	    			'<a  id="userOne" data-id='+data.id+' class="show-update button border-main" href="javascript:void(0)"><span class="icon-bars"></span>编辑/查看</a>'+
	    			'</div>'+
	    			'</td>'+
	    			'<td>'+
	    			'<div class="button-group">'+
	    			'<a data-id='+data.id+' class="show-update button border-main" href="javascript:void(0)"><span class="icon-bars"></span>导出</a>'+
	    			'</div>'+
	    			'</td>'+
	    			'<td>'+
	    			'<div class="button-group">'+
	    			'<a data-id='+data.id+' class="show-del button border-main" href="javascript:void(0)"><span class="icon-bars"></span>导出</a>'+
	    			'</div>'+
	    			'</td>'+
	    			'</tr>';
	    		});
    	...
    	init_list:function(pgn,diseaseName){
    		...
    		Base.commajax("/drugstore/user/user_list.htm","get",params,function(data){
    			console.log(data);
    			$('.sum').text(data.sum); 
...
//    			});
	...
	$(".mskldiseaseList").on("click","#userOne",function(){
		Base.getPage("pages/healthy/_userone.html");
	})
	$(".heard_search").on("click","button",function(){
		Base.getPage("pages/healthy/addUser.html");
	})
