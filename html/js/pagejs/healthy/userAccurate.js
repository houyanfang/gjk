(function ($) {
	...
	$.fn.userAccurate = function (option) {
    ...
    JqPort.prototype = {
    	...
    	initHtml:function(data,url){
    		...
    		if(data.length==0){
	    		...
	    		$.each(data, function (i, data) {
	    			...
	    			'<a  id="userTwo" data-id='+data.id+' class="show-update button border-main" href="javascript:void(0)"><span class="icon-bars"></span>编辑/查看</a>'+
	    			'</div>'+
	    			'</td>'+
	    			'<td>'+
	    			'<div class="button-group">'+
    	...
    	init_list:function(pgn,diseaseName){
    		...
    		Base.commajax("/healthy/userAccurate/userAccurate_list.htm","get",params,function(data){
    			console.log(data);
    			$('.sum').text(data.sum); 
...
//    			});
	...
	$(".mskldiseaseList").on("click","#userTwo",function(){
		Base.getPage("pages/healthy/_userTwo.html");
	})
