...
(function ($, window, document) {
    ...
    function getCalendarData(dateStr) {
        //TODO 请求计划列表
        setTimeout(function () {
            $.gjk.ajax.requestPostOrder('dailyRecord/planInfo', {
    ...
    function renderToYesterday(data, n) {
        var __a = '<a href="javascript:;" class="calendar-list-link"></a>',
            __title = '<div class="ls-title pull-left">昨日生活习惯记录</div>',
    ...
    function renderToPlan(data) {
        ...
        switch (data.logType) {
            case "sportLog" :
                __wrap = '<a href=""></a>'
                __time = '<div class="time-tag pull-left">' + "0" + deleteState(judgeWeek(data.logData))[0] + '</div>';
